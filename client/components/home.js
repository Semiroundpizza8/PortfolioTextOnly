...
export function Home() {
  ...
  return (
    <div>
      <h1 style={textStyle}>Hey there! I’m <NavLink to="/about" onMouseDown={(event) => {event.preventDefault()}}>{overlap('Benjamin Odisho', 'yellow')}</NavLink>, a Chicago-land developer who loves putting together awesome web applications. </h1>
      <h1 style={textStyle}>Wanna hear more about me and my past projects? Learn all about my life on my <NavLink to="/about" onMouseDown={(event) => {event.preventDefault()}}>{overlap('About', 'pink')}</NavLink> page, all of my different projects on my <NavLink to="/projects" onMouseDown={(event) => {event.preventDefault()}}>{overlap('Projects', 'pink')}</NavLink> page, </h1>
